...
function addButton(h1) {
  ...
  //let delurl=browser.runtime.getURL("./images/delete.png");
  let delurl =
    ...
    if (popup.style.display === "none") {
      ...
      for (const cart of cartsObj) {
        ...
        deleteButton.addEventListener("click", function () {
          ...
          browser.storage.sync.set({ Carts: cartsLoaded }, function () {
            console.log("Deleted cart from browser.storage.sync");
          });
        });
...
async function saveCart(name = "") {
  ...
  if (items.length == 0) {
    ...
    // Save the updated Carts array to browser.storage.sync
    browser.storage.sync.set({ Carts: cartsLoaded }, function () {
      console.log("Cart saved to browser.storage.sync");
    });
...
function getCarts(callback) {
  browser.storage.sync.get(["Carts"], function (items) {
